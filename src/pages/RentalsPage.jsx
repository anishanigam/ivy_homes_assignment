...
export default function RentalsPage() {
  ...
  useEffect(() => {
    async function loadRentals() {
      ...
      try {
        const b1 = await api.getRentals({
          limit: 50,
          offset: 0,
        });

        const b2 = await api.getRentals({
          limit: 50,
          offset: 50,
        });

        const b3 = await api.getRentals({
          limit: 50,
          offset: 100,
        });
  ...
  const filteredRentals = useMemo(() => {
    return rentals.filter((r) => {
      ...
      if (
        selectedBhk !== 'All' &&
        r.bedroom !== parseInt(selectedBhk, 10)
      ) {
        return false;
      }
  ...
  // Statistics for assigned locality: Malad West
  const maladStats = useMemo(() => {
    const maladItems = rentals.filter(
      (r) => r.locality?.toLowerCase() === 'malad west'
    );

    const totalRent = maladItems.reduce(
      (acc, r) => acc + (r.price || 0),
      0
    );
    ...
    const avgRent =
      maladItems.length > 0
        ? Math.round(totalRent / maladItems.length)
        : 0;

    return {
      count: maladItems.length,
      totalRent,
      avgRent,
    };
  }, [rentals]);
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm sm:text-base text-warm-600 max-w-2xl mt-1.5 leading-relaxed">
          Transparent monthly rents, explicit maintenance charges, and clear
          security deposit terms across Mumbai.
        </p>
      </div>

      {/* Highlighted Banner for Assigned Locality */}
      <div className="bg-gradient-to-r from-ivy-50 via-emerald-50 to-warm-50 border border-ivy-200 rounded-3xl p-5 sm:p-6 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div>
          <span className="text-xs font-bold text-ivy-800 uppercase tracking-wider flex items-center gap-1">
            ...
            Assigned Locality Benchmark
          </span>

          <h3 className="text-xl font-extrabold text-warm-900 mt-1">
            Malad West Rentals Hub
          </h3>

          <p className="text-xs text-warm-600 mt-0.5">
            Key metric for Question 5: Total monthly rent across all retrievable
            rental records.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 text-center">
          ...
          <div className="bg-white/80 backdrop-blur-sm px-4 py-2.5 rounded-2xl border border-ivy-200/60">
            ...
            <span className="text-base font-extrabold text-warm-900">
              ₹{maladStats.totalRent.toLocaleString('en-IN')}
            </span>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm px-4 py-2.5 rounded-2xl border border-ivy-200/60">
            ...
            <span className="text-base font-extrabold text-ivy-800">
              ₹{maladStats.avgRent.toLocaleString('en-IN')}
            </span>
          </div>
        ...
        <div className="py-16 text-center">
          <div className="bg-warm-50 border border-warm-200 rounded-2xl p-8 max-w-md mx-auto">
            ...
            <p className="text-sm text-warm-500">
              Try changing the locality or BHK filter.
            </p>
          </div>
        </div>
      )}
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[11px] text-warm-400 capitalize">
                    {r.furnishing}
                  </span>
                ...
                {/* Apartment */}
                <h3 className="font-semibold text-warm-900 text-sm mt-2">
                  {r.apartment_name}
                </h3>
                ...
                <div className="grid grid-cols-3 gap-2 py-3 border-y border-warm-100 my-3 text-xs text-warm-700">
                  ...
                  <div>
                    <span className="text-[10px] text-warm-400 block">
                      Security Deposit
                    </span>
                ...
                <p className="text-xs text-warm-500 line-clamp-2 leading-relaxed">
                  {r.description ||
                    'Convenient location with great connectivity to metro.'}
                </p>
              ...
              <div className="mt-4 pt-3 border-t border-warm-100 flex items-center justify-between text-xs">
                ...
                <span className="text-warm-400">
                  By {r.posted_by_name || 'Owner'}
                </span>

                <div className="flex items-center gap-3">

                  {/* NEW: Rental Detail Navigation */}
                  <Link
